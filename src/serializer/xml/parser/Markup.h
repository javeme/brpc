...
#define FormatString FormatMessageW
...
#define MCD_FOPEN(f,n,m) {if(_wfopen_s(&f,n,m)!=0)f=nullptr;}
...
#define FormatString FormatMessageA
#if defined(MARKUP_SAFESTR) // VC++ safe strings
#define MCD_SSZ(sz) sz,(sizeof(sz)/sizeof(MCD_CHAR))
...
#define MCD_FOPEN(f,n,m) {if(fopen_s(&f,n,m)!=0)f=nullptr;}
#else // not VC++ safe strings
#define MCD_SSZ(sz) sz
...
#endif // not STL
#define MCD_STRTOINT(s) MCD_PSZTOL(MCD_2PCSZ(s),nullptr,10)

// Allow function args to accept string objects as constant string pointers
struct MCD_CSTR
{
	MCD_CSTR() { pcsz=nullptr; };
...
{
	MCD_CSTR_FILENAME() { pcsz=nullptr; };
...
{
public:
	CMarkup() { x_InitMarkup(); SetDoc( nullptr ); };
	CMarkup( MCD_CSTR szDoc ) { x_InitMarkup(); SetDoc( szDoc ); };
	CMarkup( int nFlags ) { x_InitMarkup(); SetDoc( nullptr ); m_nDocFlags = nFlags; };
	...
	bool Parse( const MCD_STR& strDoc ){ return SetDoc(strDoc); }
	bool IsWellFormed();
	bool FindElem( MCD_CSTR szName=nullptr );
	bool FindChildElem( MCD_CSTR szName=nullptr );
	bool IntoElem();
	bool OutOfElem();
	void ResetChildPos() { x_SetPos(m_iPosParent,m_iPos,0); };
	void ResetMainPos() { x_SetPos(m_iPosParent,0,0); };
	void ResetPos() { x_SetPos(0,0,0); };
	/*<--start-->add by bluemei--*/
	int GetMainPos(){return m_iPos;};
	void SetChildPos(int iPos){ x_SetPos(m_iPosParent,m_iPos,iPos); };
	/*<--end-->add by bluemei--*/
	...
	bool AddElem( MCD_CSTR szName, MCD_CSTR szData=nullptr, int nFlags=0 ) { return x_AddElem(szName,szData,nFlags); };
	bool InsertElem( MCD_CSTR szName, MCD_CSTR szData=nullptr, int nFlags=0 ) { return x_AddElem(szName,szData,nFlags|MNF_INSERT); };
	bool AddChildElem( MCD_CSTR szName, MCD_CSTR szData=nullptr, int nFlags=0 ) { return x_AddElem(szName,szData,nFlags|MNF_CHILD); };
	bool InsertChildElem( MCD_CSTR szName, MCD_CSTR szData=nullptr, int nFlags=0 ) { return x_AddElem(szName,szData,nFlags|MNF_INSERT|MNF_CHILD); };
	...
	// Utility
	static bool ReadTextFile( MCD_CSTR_FILENAME szFileName, MCD_STR& strDoc, MCD_STR* pstrResult=nullptr, int* pnDocFlags=nullptr, MCD_STR* pstrEncoding=nullptr );
	static bool WriteTextFile( MCD_CSTR_FILENAME szFileName, const MCD_STR& strDoc, MCD_STR* pstrResult=nullptr, int* pnDocFlags=nullptr, MCD_STR* pstrEncoding=nullptr );
	...
	static MCD_STR UTF8ToA( MCD_CSTR pszUTF8, int* pnFailed = nullptr );
	static MCD_STR AToUTF8( MCD_CSTR pszANSI );
	static void EncodeCharUTF8( int nUChar, char* pszUTF8, int& nUTF8Len );
	static int DecodeCharUTF8( const char*& pszUTF8, const char* pszUTF8End = nullptr );
	static void EncodeCharUTF16( int nUChar, unsigned short* pwszUTF16, int& nUTF16Len );
	static int DecodeCharUTF16( const unsigned short*& pwszUTF16, const unsigned short* pszUTF16End = nullptr );
	static bool DetectUTF8( const char* pText, int nTextLen, int* pnNonASCII = nullptr, bool* bErrorAtEnd = nullptr );
...
#define MARKUP_SETDEBUGSTATE
	...
	/*<--start-->added by bluemei,just for search--*/
	int x_FindElem4Search( int iPosParent, int iPos, PathPos& path ) const;
